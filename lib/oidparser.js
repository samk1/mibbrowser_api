...
var syntaxClasses = require('./constants.js').OID_SYNTAX_CLASSES;
...
function parse (string) {
    ...
    if ( identifierListRe.test(string) ) {
        matches = identifierListRe.exec(string);
        syntax = {
            syntax_class: syntaxClasses.IdentifierList,
            identifier_list: parseIdentifierList(matches[1])
        };
    } else if ( moduleObjectRe.test(string) ) {
        matches = moduleObjectRe.exec(string);
        syntax = {
